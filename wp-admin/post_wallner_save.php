...
function saveCategories($category_valuesfiltered)
{
	foreach ($category_valuesfiltered as $file_id => $value) {
		// error_log("saveCategories fileid " . $file_id);
		$termsToRemove = [];
		$termIdsAdded = [];
		foreach ($value as $cf_id => $field_value) {
			$t = gettype($cf_id);
			// error_log("saveCategories cf_id " . $cf_id . ", " . print_r($field_value, true) . ", type=" . $t);
		...
		// error_log("    aveCustomFields cf_id " . $cf_id . ", ADD: " . print_r($termIdsAdded, true) . ", remove: " . print_r($termsToRemove, true));
		if (count($termsToRemove) !== 0) {
			// error_log("  Remove Categories array " . print_r($termsToRemove, true));
			$setKategoriesResult = wp_remove_object_terms($file_id, $termsToRemove, 'shared-file-category');
			// error_log("    Remove setKategoriesResult " . print_r($setKategoriesResult, true));
		}
		if (count($termIdsAdded) !== 0) {
			// error_log("  Add Categories array " . print_r($termIdsAdded, true));
			$setKategoriesResult = wp_set_post_terms($file_id, $termIdsAdded, 'shared-file-category');
			// error_log("    Insert setKategoriesResult " . print_r($setKategoriesResult, true));
